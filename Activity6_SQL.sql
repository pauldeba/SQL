...
create table Debashis_Activities.orders(
    order_no int primary key, purchase_amount float, order_date date,
    customer_id int, salesman_id int);

-- Add values to the table
insert into Debashis_Activities.orders values
...
select distinct salesman_id from Debashis_Activities.orders;
-- Display the order number ordered by date in ascending order
select order_no,order_date from Debashis_Activities.orders order by order_date;
-- Display the order number ordered by purchase amount in descending order
select order_no,purchase_amount from Debashis_Activities.orders order by purchase_amount desc;
-- Display the full data of orders that have purchase amount less than 500.
select * from Debashis_Activities.orders where purchase_amount<500;
-- Display the full data of orders that have purchase amount between 1000 and 2000.
select * from Debashis_Activities.orders where purchase_amount between 1000 and 2000;
